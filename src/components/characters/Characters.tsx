...
export function Characters({ IgrapqlconnectiontoPeople }: Props): JSX.Element {
  ...
  const [characters, setCharacters] = useState<Array<ICharacter>>(
    IgrapqlconnectiontoPeople?.allPeople?.people ?? [],
  );
  const [nextPage, setNextPage] = useState<string | null>(
    IgrapqlconnectiontoPeople?.allPeople?.pageInfo.endCursor ?? '',
  );
  ...
  const fetchMorePeople = async (): Promise<void> => {
    ...
    if (!result.ok) {
      ...
      throw new Error(
        `Error fetching from SWAPI, non 200 status from character file:
         ${result.status}`,
      );
    }
